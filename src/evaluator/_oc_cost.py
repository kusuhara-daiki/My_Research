...
def cost_func(x, y, mode: str = "giou", alpha: float = 0.8):
    ...
    Args:
        x (np.ndarray): a detection [x1, y1, x2, y2, s, l].
            s is a confidence value, and l is a classification label.
        y (np.ndarray): a detection [x1, y1, x2, y2, s, l].
            s is a confidence value, and l is a classification label.
        mode (str, optional): Type of IoUs.
            Defaults to "giou" (Generalized IoU).
        alpha (float, optional): weights to balance localization
            and classification errors. Defaults to 0.8.
...
def get_cmap(
    ...
    def metric(x, y):
        return cost_func(x, y, alpha=alpha, mode=mode)
...
def postprocess(M: npt.ArrayLike, P: npt.ArrayLike) -> float:
    """drop dummy to dummy costs, normalize the transportation plan,
...
def get_ot_cost(
    ...
    Args:
        a_detection (list): list of detection results.
            a_detection[i] contains bounding boxes for i-th class.
        Each element is numpy array whose shape is N x 5.
            [[x1, y1, x2, y2, s], ...]
        b_detection (list): ditto
        costmap_func (callable): a function that takes a_detection
            and b_detection as input and returns a unit cost matrix
    Returns:
